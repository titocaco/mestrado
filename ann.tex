...
\begin{equation}
    \varphi \left(\bm{x}\right) = \nsum_{i=1}^{N} \alpha_{i}\ \rho\left(\ \norm{\ \bm{x} - \bm{c}_{i}\ }\ \right)
    \label{eq:ann_rbf_output}
    \hspace{0.1cm},
    \vspace{0.2cm}
\end{equation}

\noindent com $N$ sendo o número de neurônios da camada intermediária (oculta); $\bm{c}_{i}$ e $\alpha_{i}$ são, respectivamente, o vetor centro e o peso sináptico para o neurônio $i$. E, tipicamente, a função de base radial utilizada é Gaussiana

\begin{equation}
    \rho\left(\ \norm{\ \bm{x} - \bm{c}_{i}\ }\ \right) = \exp{\left(-\beta\ \norm{\ \bm{x} - \bm{c}_{i}\ }^{\ 2}\right)}
